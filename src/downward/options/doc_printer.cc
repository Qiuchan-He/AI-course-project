#include "downward/options/doc_printer.h"
...
void DocPrinter::print_section(
    ...
    const vector<PluginInfo>& infos)
{
    if (!group_id.empty()) {
        const PluginGroupInfo& group = registry.get_group_info(group_id);
...
{
    os << "== " << info.name << " ==" << endl;
    if (!info.synopsis.empty()) os << info.synopsis << endl;
}
...
void Txt2TagsPrinter::print_usage(const string& name, const PluginInfo& info)
{
    if (!name.empty()) {
        ...
        for (size_t i = 0; i < info.arg_help.size(); ++i) {
            ...
            os << arg.key;
            if (!arg.default_value.empty()) os << "=" << arg.default_value;
            if (i != info.arg_help.size() - 1) os << ", ";
        }
        os << ")" << endl << endl << endl;
    }
}
    ...
    const string& alias)
{
    if (!predefinition_key.empty()) {
        os << endl
           << "This plugin type can be predefined using ``--"
    ...
    const string& alias)
{
    if (!predefinition_key.empty()) {
        os << endl
           << "This plugin type can be predefined using --" << predefinition_key
           << "." << endl;
    }
    if (!alias.empty()) {
        os << "The old predefinition key --" << alias << " is still "
