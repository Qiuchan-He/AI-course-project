...
#include "downward/ext/tree_util.hh"
    ...
    T upper_bound)
{
    ...
    if (value < lower_bound || value > upper_bound) {
        ...
        stream << key << " (" << value << ") must be in range [" << lower_bound
               << ", " << upper_bound << "]";
...
ParseTree generate_parse_tree(const string& config)
{
    ...
    for (size_t i = 0; i < config.size(); ++i) {
        ...
        if ((next == '(' || next == ')' || next == ',') && !buffer.empty()) {
            ...
            throw ParseError(
                "misplaced opening bracket (",
                *cur_node,
                config.substr(0, i));
        }
        switch (next) {
        case ' ': break;
        case '(': cur_node = last_child(tree, cur_node); break;
        case ')':
    ...
    if (!buffer.empty()) tree.append_child(cur_node, ParseNode(buffer, key));
...
Options OptionParser::parse()
{
    ...
    for (auto tree_it = first_child_of_root(parse_tree);
        ...
        if (!tree_it->key.empty()) {
            if (find(valid_keys.begin(), valid_keys.end(), tree_it->key) ==
                valid_keys.end()) {
                error(
                    "invalid keyword " + tree_it->key + " for " +
                    get_root_value());
            }
        }
...
void OptionParser::document_synopsis(const string& name, const string& note)
    const
{
    registry.set_plugin_info_synopsis(get_root_value(), name, note);
}

void OptionParser::document_property(const string& property, const string& note)
    const
{
