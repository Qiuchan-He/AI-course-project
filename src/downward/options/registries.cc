...
#include "downward/options/errors.h"
...
Registry::Registry(const RawRegistry& raw_registry)
{
    ...
    // The documentation generation requires an error free, fully initialized
    // registry.
...
void Registry::insert_plugin_types(
    ...
    vector<string>& errors)
{
    ...
    for (const PluginTypeInfo& plugin_type_info :
        ...
        occurrences_names[plugin_type_info.type_name].push_back(
            plugin_type_info.type);
        occurrences_types[plugin_type_info.type].push_back(
    ...
    for (auto& it : occurrences_predefinition) {
        if (it.second.size() > 1) {
            errors.push_back(
                "Multiple PluginTypePlugins use the predefinition "
                "key " +
                it.first + " (types: " + utils::join(it.second, ", ") + ")");
        }
    }
}

void Registry::insert_plugin_groups(
    const RawRegistry& raw_registry,
    vector<string>& errors)
{
    ...
    for (auto& it : occurrences) {
        if (it.second > 1) {
            errors.push_back(
                "Multiple definitions (" + to_string(it.second) +
                ") for PluginGroupPlugin " + it.first);
        }
    }
}

void Registry::insert_plugins(
    const RawRegistry& raw_registry,
    vector<string>& errors)
{
    ...
    for (const RawPluginInfo& plugin : raw_registry.get_plugin_data()) {
        ...
        if (!plugin.group.empty() && !plugin_group_infos.count(plugin.group)) {
            errors.push_back(
                ...
                " for Plugin " + plugin.key + " of type " + plugin.type.name());
            error = true;
        }
...
void Registry::insert_type_info(const PluginTypeInfo& info)
{
    ...
    for (const string& predefinition_key :
        ...
        if (!predefinition_key.empty()) {
            ...
            predefinition_functions[predefinition_key] =
                info.predefinition_function;
        }
    }
...
const PluginTypeInfo& Registry::get_type_info(const type_index& type) const
{
    if (!plugin_type_infos.count(type)) {
        ABORT(
            "attempt to retrieve non-existing type info from registry: " +
            string(type.name()));
    }
    ...
    const type_index& type)
{
    ...
    PluginInfo doc;
    doc.key = key;
    // Plugin names can be set with document_synopsis. Otherwise, we use the
    // key.
    doc.name = key;
...
void Registry::handle_all_repredefinition(
    Predefinitions& predefinitions,
    std::unordered_set<string> ign)
{
    for (auto& predef : predefinitions_raw_ordered) {
        if (ign.count(predef.key) == 0) {
