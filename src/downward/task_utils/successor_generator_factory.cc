...
GeneratorPtr
SuccessorGeneratorFactory::construct_fork(vector<GeneratorPtr> nodes) const
{
    int size = nodes.size();
    if (size == 1) {
        return std::move(nodes.at(0));
    } else if (size == 2) {
        return std::make_unique<GeneratorForkBinary>(
            std::move(nodes.at(0)),
            std::move(nodes.at(1)));
    } else {
        /* This general case includes the case size == 0, which can
           (only) happen for the root for tasks with no operators. */
        return std::make_unique<GeneratorForkMulti>(std::move(nodes));
    }
}

GeneratorPtr
SuccessorGeneratorFactory::construct_leaf(OperatorRange range) const
{
    assert(!range.empty());
    ...
    if (operators.size() == 1) {
        return std::make_unique<GeneratorLeafSingle>(operators.front());
    } else {
        return std::make_unique<GeneratorLeafVector>(std::move(operators));
    }
}
    ...
    ValuesAndGenerators values_and_generators) const
{
    ...
    if (num_children == 1) {
        ...
        return std::make_unique<GeneratorSwitchSingle>(
            switch_var_id,
            value,
            std::move(generator));
    }
    ...
    if (hash_bytes < vector_bytes) {
        ...
        return std::make_unique<GeneratorSwitchHash>(
            switch_var_id,
        ...
        for (auto& item : values_and_generators)
            generator_by_value[item.first] = std::move(item.second);
        return std::make_unique<GeneratorSwitchVector>(
            switch_var_id,
            std::move(generator_by_value));
    }
}
...
GeneratorPtr
SuccessorGeneratorFactory::construct_recursive(int depth, OperatorRange range)
    const
{
...
build_sorted_precondition(const AbstractOperatorProxy& op)
{
    vector<FactPair> precond;
    precond.reserve(op.get_precondition().size());
    for (FactProxy pre : op.get_precondition())
...
{
    AbstractOperatorsProxy operators = task_proxy.get_abstract_operators();
    operator_infos.reserve(operators.size());
    for (AbstractOperatorProxy op : operators) {
