...
class MaxCliqueComputer {
    ...
    set<set<int>>& max_cliques;
    set<int> current_max_clique;

    int get_maximizing_vertex(const vector<int>& subg, const vector<int>& cand)
    {
    ...
    void expand(vector<int>& subg, vector<int>& cand)
    {
        ...
        if (subg.empty()) {
            // cout << "clique" << endl;
            max_cliques.insert(current_max_clique);
        } else {
...
public:
    MaxCliqueComputer(
        const vector<set<int>>& graph_,
        set<set<int>>& max_cliques_)
        : graph(graph_)
...
set<set<int>> compute_max_cliques(const vector<set<int>>& graph)
{
    set<set<int>> max_cliques;
