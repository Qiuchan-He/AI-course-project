#include "neuralfd/search_algorithms/policy_walk.h"
...
PolicyWalk::PolicyWalk(const Options& opts)
    : SearchAlgorithm(opts)
    , policy(opts.get<shared_ptr<Policy>>("policy"))
    , dead_end_evaluators(
          opts.get_list<shared_ptr<Evaluator>>("dead_end_evaluators"))
...
{
    return any_of(
        dead_end_evaluators.begin(),
        dead_end_evaluators.end(),
...
SearchStatus PolicyWalk::step()
{
    ...
    vector<OperatorID> operator_ids =
        eval_context.get_preferred_operators(policy.get());
    vector<float> operator_prefs =
        eval_context.get_operator_preferences(policy.get());
    ...
    while (!operator_ids.empty()) {
        ...
        if (op_select == OperatorSelection::Probability) {
            ...
            op_select == OperatorSelection::First ||
        ...
        eval_context = EvaluationContext(
            succ_state,
            current_node.get_g() + get_adjusted_cost(op_proxy),
            true,
            &statistics);
        if (succ_node.is_new()) {
        ...
        if (succ_node.is_new()) {
            ...
            cerr << "Internal error. An open search node should not be"
                    "reached in the policy walk"
                 << endl;
            utils::exit_with(ExitCode::SEARCH_CRITICAL_ERROR);
        }

        current_state = std::move(succ_state);
        trajectory_length++;
        return IN_PROGRESS;
    }
...
static shared_ptr<SearchAlgorithm> _parse(OptionParser& parser)
{
    ...
    if (!parser.dry_run()) {
        algorithm = make_shared<PolicyWalk>(opts);
    }
    return algorithm;
}

static Plugin<SearchAlgorithm> _plugin("policy_walk", _parse);
} // namespace search_algorithms
