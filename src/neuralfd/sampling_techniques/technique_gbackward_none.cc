#include "neuralfd/sampling_techniques/technique_gbackward_none.h"

#include "neuralfd/tasks/modified_init_goals_task.h"
#include "neuralfd/tasks/partial_state_wrapper_task.h"

#include "neuralfd/task_utils/regression_task_proxy.h"
#include "neuralfd/task_utils/sampling.h"
...
{
    auto pair_success_state = assignment.get_full_state(true, rng);
    ...
    State state = state_registry.insert_state(std::move(initial_facts));
...
std::shared_ptr<ClassicalTask> TechniqueGBackwardNone::create_next(
    ...
    const ClassicalTaskProxy& task_proxy)
{
    if (seed_task != last_task) {
        ...
        rrws = make_shared<sampling::RandomRegressionWalkSampler>(
    ...
    if (!bias_evaluator_tree.empty() &&
        ...
        options::OptionParser bias_parser(
            bias_evaluator_tree,
            *registry,
            *predefinitions,
            false);
    ...
    auto is_valid_state = [&](PartialAssignment& partial_assignment) {
        return !(is_valid_walk) ||
               regression_task_proxy
                   ->convert_to_full_state(partial_assignment, true, *rng)
                   .first;
    };
    ...
    while (true) {
        PartialAssignment partial_assignment = rrws->sample_state_length(
        ...
        auto complete_assignment =
            partial_assignment.get_full_state(true, *rng);
        ...
        if (wrap_partial_assignment) {
            if (last_task != seed_task) {
                last_partial_wrap_task =
                    make_shared<extra_tasks::PartialStateWrapperTask>(
                        seed_task);
            }
            ...
            return make_shared<extra_tasks::ModifiedInitGoalsTask>(
                last_partial_wrap_task,
                std::move(new_init_values),
                extractGoalFacts(regression_task_proxy->get_goal()));
        } else {
            return make_shared<extra_tasks::ModifiedInitGoalsTask>(
                seed_task,
...
void TechniqueGBackwardNone::dump_upgradable_parameters(
...
static shared_ptr<TechniqueGBackwardNone>
_parse_technique_gbackward_none(options::OptionParser& parser)
{
...
static Plugin<SamplingTechnique> _plugin_technique_gbackward_none(
    TechniqueGBackwardNone::name,
    _parse_technique_gbackward_none);
