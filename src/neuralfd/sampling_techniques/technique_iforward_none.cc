#include "neuralfd/sampling_techniques/technique_iforward_none.h"
...
std::shared_ptr<ClassicalTask> TechniqueIForwardNone::create_next(
    ...
    const ClassicalTaskProxy& task_proxy)
{
    ...
    // add option bias, deprioritize undoing steps, bias probabilistic,
    // bias_adapt
    ...
    return make_shared<extra_tasks::ModifiedInitGoalsTask>(
        ...
        extractGoalFacts(task_proxy.get_goal()));
}

/* PARSING TECHNIQUE_IFORWARD_NONE*/
static shared_ptr<SamplingTechnique>
_parse_technique_iforward_none(options::OptionParser& parser)
{
...
static Plugin<SamplingTechnique> _plugin_technique_iforward_none(
    TechniqueIForwardNone::name,
    _parse_technique_iforward_none);
