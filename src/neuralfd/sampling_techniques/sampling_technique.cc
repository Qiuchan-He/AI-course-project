 ...
 * We use this to instantiate search algorithms with our desired modified tasks.
 */
std::shared_ptr<ClassicalTask> modified_task = nullptr;

static shared_ptr<ClassicalTask>
_parse_sampling_transform(options::OptionParser& parser)
{
    if (parser.dry_run()) {
        return nullptr;
    } else {
...
static Plugin<ClassicalTask>
    _plugin_sampling_transform("sampling_transform", _parse_sampling_transform);
...
// REDUNDANCY
static bool are_facts_mutex(
    const vector<vector<set<FactPair>>>& mutexes,
...
SamplingTechnique::SamplingTechnique(const options::Options& opts)
    ...
    , option_parser(std::make_unique<OptionParser>(
          eval_parse_tree,
          *opts.get_registry(),
          *opts.get_predefinitions(),
          false,
          false))
...
shared_ptr<ClassicalTask>
SamplingTechnique::next(const shared_ptr<ClassicalTask>& seed_task)
{
    return next(seed_task, ClassicalTaskProxy(*seed_task));
}

shared_ptr<ClassicalTask> SamplingTechnique::next(
    const shared_ptr<ClassicalTask>& seed_task,
    const ClassicalTaskProxy& task_proxy)
{
    if (empty()) {
        ...
        counter++;
        while (true) {
            shared_ptr<ClassicalTask> next_task =
                create_next(seed_task, task_proxy);
            modified_task = next_task;
            if ((check_mutexes && !test_mutexes(next_task)) ||
                (check_solvable &&
                 !test_solvable(ClassicalTaskProxy(*next_task)))) {
...
void SamplingTechnique::do_upgrade_parameters()
{
    cerr << "Either this sampling technique has not upgradable parameters"
            "or the upgrade method has to be implemented."
         << endl;
...
{
    cerr << "Either this sampling technique has not upgradable parameters"
            "or the dump parameter method has to be implemented."
         << endl;
    utils::exit_with(utils::ExitCode::SEARCH_CRITICAL_ERROR);
}

bool SamplingTechnique::test_mutexes(
    const shared_ptr<ClassicalTask>& task) const
{
    // Check initial state
    ...
    for (int i = 0; i < task->get_num_goal_facts(); ++i) {
        ...
        for (int j = i + 1; j < task->get_num_goal_facts(); ++j) {
...
bool SamplingTechnique::test_solvable(
    const ClassicalTaskProxy& task_proxy) const
{
...
//         std::shared_ptr<ClassicalTask> /*task*/) const {
...
vector<FactPair>
SamplingTechnique::extractGoalFacts(const GoalProxy& goals_proxy)
{
...
static PluginTypePlugin<SamplingTechnique>
    _type_plugin("SamplingTechnique", "Generates from a given task a new one.");
} // namespace sampling_technique
