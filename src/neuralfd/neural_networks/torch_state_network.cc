#include "neuralfd/neural_networks/torch_state_network.h"
...
TorchStateNetwork::TorchStateNetwork(const Options& opts)
    ...
    , relevant_facts(
          get_fact_mapping(task.get(), opts.get_list<string>("facts")))
    , default_input_values(
          get_default_inputs(opts.get_list<string>("defaults")))
{
...
vector<at::Tensor> TorchStateNetwork::get_input_tensors(const State& state)
{
    ...
    at::Tensor tensor =
        torch::ones({1, static_cast<long>(relevant_facts.size())});
...
void TorchStateNetwork::parse_output(const torch::jit::IValue& output)
{
    ...
    for (int64_t i = 0; i < tensor.size(0); ++i) {
        last_h = static_cast<int>(
            (accessor[i][0] + heuristic_shift) * heuristic_multiplier);
...
static Plugin<neural_networks::AbstractNetwork>
    _plugin("torch_state_network", _parse);
