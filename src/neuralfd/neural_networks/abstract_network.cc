...
void AbstractNetwork::verify_heuristic_confidence()
{
    if (!is_heuristic_confidence()) {
        cerr << "Network does not support heuristic confidence estimates."
             << endl
             << "Terminating." << endl;
        utils::exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
    }
...
{
    cerr << "Network does not support confidences for heuristic estimates."
         << endl
         << "Terminating." << endl;
    utils::exit_with(utils::ExitCode::SEARCH_UNSUPPORTED);
}
...
std::vector<FactPair>
get_fact_mapping(const ClassicalTask* task, const vector<string>& facts)
{
    if (facts.empty()) {
    ...
    for (const FactPair& fp : task_properties::get_strips_fact_pairs(task)) {
        ...
        fact_name =
            options::stringify_tree(options::generate_parse_tree(fact_name));
...
std::vector<int>
get_default_inputs(const std::vector<std::string>& default_inputs)
{
    vector<int> converted;
    converted.reserve(default_inputs.size());
    transform(
        default_inputs.begin(),
        default_inputs.end(),
    ...
    const vector<int>& default_input_values)
{
    if (default_input_values.empty()) {
        if (any_of(
                relevant_facts.begin(),
                relevant_facts.end(),
                [](const FactPair& fp) { return fp == FactPair::no_fact; })) {
        ...
        if (default_input_values.size() != relevant_facts.size()) {
            ...
            cerr
                << "If default values are given for the facts, then exactly one"
                   "value has to be given for every (STRIPS) fact (facts: "
                << relevant_facts.size()
                << ", default values: " << default_input_values.size() << endl;
            utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
        }
    }
}
