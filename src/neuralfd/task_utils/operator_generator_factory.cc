...
GeneratorPtr
OperatorGeneratorFactory::construct_fork(vector<GeneratorPtr> nodes) const
{
    int size = nodes.size();
    if (size == 1) {
...
GeneratorPtr OperatorGeneratorFactory::construct_leaf(OperatorRange range) const
{
    assert(!range.empty());
...
GeneratorPtr
OperatorGeneratorFactory::construct_recursive(int depth, OperatorRange range)
    const
{
