...
{
    OperatorID op_id1 =
        generator1->generate_min_applicable_op(state, reject_unassigned);
    OperatorID op_id2 =
        generator2->generate_min_applicable_op(state, reject_unassigned);
    ...
    bool reject_unassigned) const
{
    ...
    for (const auto& generator : children) {
        OperatorID op_id =
            generator->generate_min_applicable_op(state, reject_unassigned);
    ...
    bool reject_unassigned) const
{
    ...
    if (val != -1) {
        ...
        if (generator_for_val) {
            return generator_for_val->generate_min_applicable_op(
                state,
                reject_unassigned);
        }
        return OperatorID(-1);
        ...
        if (covers_all_values) {
            ...
            for (auto& generator_for_val : generator_for_value) {
                if (generator_for_val) {
                    OperatorID op_id =
                        generator_for_val->generate_min_applicable_op(
                            state,
                            reject_unassigned);
    ...
    if (val != -1) {
        ...
        if (covers_all_values) {
            ...
            for (auto& iter : generator_for_value) {
                OperatorID op_id = iter.second->generate_min_applicable_op(
                    state,
                    reject_unassigned);
                max_id = max_op_id(max_id, op_id);
            }
...
{
    int val = state[switch_var_id];
    if ((val == -1 && (!reject_unassigned || covers_all_values)) ||
        value == val) {
        return generator_for_value->generate_min_applicable_op(
            state,
            reject_unassigned);
    }
    return OperatorID(-1);
...
void GeneratorSwitchSingle::generate_applicable_ops(
    ...
    vector<OperatorID>& applicable_ops) const
{
    if (value == state[switch_var_id] || state[switch_var_id] == -1) {
...
OperatorID
GeneratorLeafVector::generate_min_applicable_op(const vector<int>&, bool) const
{
    return (applicable_operators.empty()) ? OperatorID(-1)
                                          : applicable_operators.back();
}
...
OperatorID
GeneratorLeafSingle::generate_min_applicable_op(const vector<int>&, bool) const
{
