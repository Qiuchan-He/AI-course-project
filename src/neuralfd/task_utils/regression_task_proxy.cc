...
bool RegressionCondition::is_satisfied(
    const PartialAssignment& assignment) const
{
    return !assignment.assigned(data.var) ||
           data.value == assignment[data.var].get_value();
    // int current_val = assignment[data.var].get_value();
    // return current_val == PartialAssignment::UNASSIGNED || current_val ==
    // data.value;
}

RegressionConditionProxy::RegressionConditionProxy(
...
RegressionEffectProxy::RegressionEffectProxy(
    const ClassicalTask& task,
    const RegressionEffect& effect)
    : task(&task)
    , effect(effect)
{
}
...
bool RegressionOperator::achieves_subgoal(
    const PartialAssignment& assignment) const
{
    return any_of(
        original_effect_vars.begin(),
        original_effect_vars.end(),
        [&](int var_id) { return assignment.assigned(var_id); });
}

bool RegressionOperator::is_applicable(
    const PartialAssignment& assignment) const
{
...
inline shared_ptr<vector<RegressionOperator>>
extract_regression_operators(ClassicalTaskProxy& tp)
{
...
RegressionTaskProxy::RegressionTaskProxy(const ClassicalTask& task)
    : ClassicalTaskProxy(task)
    , operators(extract_regression_operators(*this))
{
}
