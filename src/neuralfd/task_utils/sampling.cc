...
bool sample_next_state(
    ...
    double& current_bias,
    const predecessor_generator::PredecessorGenerator& generator,
    ...
    const PartialDeadEndDetector& is_dead_end)
{
    ...
    const function<
        PartialAssignment(const PartialAssignment&, const OperatorID&)>
        construct_candidate =
            [&](const PartialAssignment& s,
                const OperatorID& op_id) -> PartialAssignment {
        RegressionOperatorProxy op_proxy =
            regression_task_proxy.get_regression_operator(op_id);
...
RandomRegressionWalkSampler::RandomRegressionWalkSampler(
    ...
    , predecessor_generator(
          std::make_unique<predecessor_generator::PredecessorGenerator>(
              regression_task_proxy))
    , goals(regression_task_proxy.get_goal_assignment())
    , average_operator_costs(
          task_properties::get_average_operator_cost(regression_task_proxy))
    , rng(rng)
{
}
...
std::pair<PartialAssignmentRegistry, utils::HashMap<size_t, int>>
RandomRegressionWalkSampler::sample_area(
    ...
    bool check_mutexes) const
{
    ...
    while (!open.empty()) {
        ...
        if (cur_cost > max_cost) {
            cout << "Hit cost limit. Expanded states: " << id2cost.size()
                 << endl;
            break;
        }
        ...
        if (max_states >= 0 && id2cost.size() >= (size_t)max_states) {
            cout << "Hit state limit. Expanded states: " << id2cost.size()
                 << endl;
            break;
        }
        ...
        for (OperatorID& op_id : applicable_operators) {
            RegressionOperatorProxy next_op =
                regression_task_proxy.get_regression_operator(op_id);
            assert(task_properties::is_applicable(next_op, cur_assignment));
            PartialAssignment next_assignment =
                next_op.get_anonym_predecessor(cur_assignment);
            if (check_mutexes) {
                if (next_assignment.violates_mutexes()) {
                    continue;
                }
                //                auto next_state =
                //                next_assignment.get_full_state(check_mutexes,
                //                rng); if (!next_state.first) {
                //                    continue;
                //                }
            }
            size_t next_id =
                registry.lookup_or_insert_by_assignment(next_assignment);

            int next_cost = cur_cost + next_op.get_cost();
            open.emplace(next_cost, next_id);
        }
    }
    return {std::move(registry), std::move(id2cost)};
}
} // namespace sampling
