...
#include "neuralfd/policy_result.h"
...
PolicyResult Policy::compute_result(EvaluationContext& eval_context)
{
    ...
    if (cache_policy_values && !policy_cache[state].dirty) {
        PolicyResult result;
        result.set_preferred_operators(
            vector<OperatorID>(policy_cache[state].operator_ids));
        result.set_operator_preferences(
            vector<float>(policy_cache[state].operator_preferences));
...
void Policy::add_options_to_parser(OptionParser& parser)
{
    parser.add_option<shared_ptr<ClassicalTask>>(
        ...
        "available.",
        "no_transform()");
    //    parser.add_option<string>("register", "Registers a policy pointer by
    //    a"
    //        "given name on the task object.", "None");
    parser.add_option<bool>(
        "cache_estimates",
        "cache policy estimates",
        "true");
}
