...
static size_t
calculate_modification_hash(const State& init, const GoalProxy& goals)
{
    ostringstream oss;
    ...
    const string& snd_state)
{
    return "{" + entry_meta_general + to_string(modification_hash) +
           ...
           sample_type +
           "\", "
           "\"fields\": [{\"name\": \"current_state\", \"type\": \"state\", "
           "\"format\": \"FD\"}, " +
           ((skip_goal_field)
                ? ""
                : R"({"name": "goals", "type": "state", "format": "FD"}, )") +
           ((skip_snd_state_field)
                ? ""
                : (R"({"name": ")" + snd_state +
                   R"(", "type": "state", "format": "FD"}, )")) +
           ((skip_action_field) ? ""
                                : R"({"name": "action", "type": "string"}, )") +
    ...
    const StateRegistry& sr)
{
    ...
    if (sample_format == SampleFormat::FIELDS) {
        ...
        if (!skip_snd_state_field) {
            ...
            if (second_state_id != StateID::no_state) {
                convert_and_push_state(
                    stream,
                    sr.lookup_state(second_state_id));
            }
        }
        ...
        if (add_unsolved_samples) {
            stream << field_separator;
            stream << !algorithm->found_solution();
        }
        ...
        if (add_unsolved_samples) {
            stream << algorithm->found_solution() << field_separator;
        }
...
int SamplingSearch::extract_entries_trajectories(
    ...
    const StateRegistry& sr,
    const OperatorsProxy& ops,
    const string& meta_init,
    const string& meta_inter,
    const Plan& plan,
    ...
    const string& pddl_goal)
{
    ...
    vector<int> intermediate =
        rng->choose_n_of_N(store_intermediate, trajectory.size());
    ...
    for (size_t idx_goal = trajectory.size(); idx_goal-- > min_idx_goal;) {
        if (pddl_goal.empty() || expand) {
            // TODO: Goal is to precise, can we make it more exact via
            // Regression?
            ostringstream stream_pddl_goal;
            convert_and_push_state(
                stream_pddl_goal,
                sr.lookup_state(trajectory[idx_goal]));
        ...
        for (size_t idx_init = idx_goal + 1; idx_init-- > 0;) {
            ...
            if (!(store_trajectory || (store_init && idx_init == 0) ||
                  (store_intermediate &&
                   std::find(
                       intermediate.begin(),
                       intermediate.end(),
                       idx_init) != intermediate.end()))) {
                continue;
            }

            const State init = sr.lookup_state(trajectory[idx_init]);
            const StateID next_state_id = (idx_init == idx_goal)
                                              ? StateID::no_state
                                              : trajectory[idx_init + 1];
            const OperatorID& op_id = (idx_init == idx_goal)
            ...
            if (idx_goal == trajectory.size() - 1) {
                ...
                for (size_t idx_h = 0; idx_h < ptr_use_evaluators.size();
                     ++idx_h) {
            ...
            add_entry(
                new_entries,
                (idx_init == 0) ? meta_init : meta_inter,
                init,
                string_pddl_goal,
                next_state_id,
                op_id,
...
static string construct_sample_file_header(
    ...
    const string& field_separator)
{
    ...
    if (sample_format == SampleFormat::FIELDS) {
        header << SAMPLE_FILE_MAGIC_WORD << endl
               ...
               << "#\tmodification_hash: has for the modification done on the "
                  "initial state and goal"
               << endl
               << "#\tdelimiter: string used to separate the different fields "
                  "of the entry"
               << endl
               << "#\tsample_type: describes where this entry comes from. "
                  "Available:"
               ...
               << "#\tfields: List of one dictionary per field to describe the "
                  "data entry"
               << endl
               << "# General Field attributes:" << endl
               << "#\tname: name for the field" << endl
               << "#\ttype: type of the field. Available types: \"state\", "
                  "\"heuristic\", \"int\", \"string\""
               << endl
               << "#State Field attributes:" << endl
               << "#\tformat: format in which the state is stored, e.g. \"FD\" "
                  "for the format Fast Downward uses"
               << endl;
    } else {
        utils::exit_with(utils::ExitCode::SEARCH_CRITICAL_ERROR);
    }
...
static string construct_meta_general(
    const string& field_separator,
...
static string construct_meta_heuristics(const vector<string>& use_evaluators)
{
...
SamplingSearch::SamplingSearch(const options::Options& opts)
    ...
    , constructed_sample_file_header(
          sample_format == SampleFormat::FIELDS
              ...
              : SamplingStateAlgorithm::sample_file_header())
    , entry_meta_general(construct_meta_general(field_separator, problem_hash))
    ...
    , successfully_solved_increment_threshold(static_cast<int>(
          successfully_solved_history_size *
          opts.get<double>("upgrade_solved_rate")))
{
    if (sample_format != SampleFormat::FIELDS &&
    ...
    if (state_format != StateFormat::PDDL && state_format != StateFormat::FDR) {
    ...
    if (store_expansions && (store_solution_trajectories ||
                             store_other_trajectories || store_all_states)) {
...
namespace {
template <typename Facts, typename T>
void dump_pddl(const Facts& facts, T& out, const std::string& separator)
{
...
vector<string> SamplingSearch::extract_samples()
{
    ...
    const StateRegistry& sr = algorithm->get_state_registry();
    const SearchSpace& ss = algorithm->get_search_space();
    const ClassicalTaskProxy& tp = algorithm->get_task_proxy();
    OperatorsProxy ops = tp.get_operators();
    const GoalProxy gps = tp.get_goal();
    size_t mod_hash = calculate_modification_hash(tp.get_initial_state(), gps);

    vector<string> new_entries;

    if (store_expansions) {
        if (algorithm->found_solution() || store_expansions_unsolved) {
            ostringstream stream_pddl_goal;
            dump_pddl(gps, stream_pddl_goal, "\t");
            vector<int> expansions = {
                algorithm->get_statistics().get_expanded()};
            add_entry(
                new_entries,
                construct_meta(mod_hash, "init", "next_state"),
                tp.get_initial_state(),
                stream_pddl_goal.str(),
                StateID::no_state,
                OperatorID::no_operator,
                &expansions,
                ops,
    ...
    if (store_solution_trajectories) {
        if (algorithm->found_solution()) {
            ...
            ss.trace_path(algorithm->get_goal_state(), trajectory);
    ...
    if (store_other_trajectories) {
        const string meta_init =
            construct_meta(mod_hash, "trajectory_init", "next_state");
        const string meta_inter =
            construct_meta(mod_hash, "trajectory_inter", "next_state");

        for (const Path& path : sampling_algorithm::paths) {
            extract_entries_trajectories(
                new_entries,
                sr,
                ops,
    ...
    if (store_all_states) {
        const string meta_visited =
            construct_meta(mod_hash, "visited", "previous_state");
        ostringstream stream_pddl_goal;
        dump_pddl(gps, stream_pddl_goal, "\t");
        const string pddl_goal = stream_pddl_goal.str();

        for (StateID state : sr) {
...
void SamplingSearch::next_algorithm()
{
    ...
    SamplingSearchBase::next_algorithm();
...
void SamplingSearch::post_search(std::vector<std::string>& samples)
{
    if ((*current_technique)->has_upgradeable_parameters()) {
        auto& history =
            successfully_solved_history.find((*current_technique)->id)->second;
        size_t nb_solved =
            successfully_solved.find((*current_technique)->id)->second;
        history.push_back(algorithm->found_solution());
        if (algorithm->found_solution()) {
            nb_solved++;
        }
...
void SamplingSearch::add_sampling_search_options(options::OptionParser& parser)
{
    ...
    parser.add_option<bool>(
        "store_initial_state",
        "Stores for the initial state itself, its cost "
        "(estimated by the plan found), the operator selected as well as the "
        "goal "
        "and the heuristic estimates of the heuristics registered via "
        "\"evaluators\" for each trajectory given (solution_trajectory and "
        "other_trajectories). This prevents storing the whole trajectory.",
        "true");

    parser.add_option<bool>(
        "store_intermediate_state",
        "Stores a random uniformly chosen state per "
        ...
        "storing the whole trajectory.",
        "false");

    parser.add_option<bool>(
        "expand_trajectory",
        "Stores for every state combination on a trajectory which operator was "
        "chosen"
        "next to from the first to the second state, the next state reached, "
        "and the heuristic "
        "values estimated for the state.",
        "false");
    ...
    parser.add_option<bool>(
        ...
        "with storing anything except the initial state of the sampling "
        "search.",
        "false");
    parser.add_option<bool>(
        "store_expansions_unsolved",
    ...
    parser.add_option<bool>(
        "skip_goal_field",
        "does not store the goal "
        "field of an entry.",
        "false");
    parser.add_option<bool>(
        "skip_second_state_field",
        "does not store the second state "
        "(depending on entry type successor or predecessor) field of "
        "an entry.",
        "false");
    parser.add_option<bool>(
        "skip_action_field",
        "does not store the selected action "
        "field of an entry.",
        "false");
    parser.add_option<bool>(
        "add_unsolved_samples",
    ...
    parser.add_list_option<std::string>(
    ...
    parser.add_option<std::string>(
        ...
        "differentiate which problems created which entries.",
        "none");
    ...
    parser.add_option<int>(
        ...
        "0 = always, N > 0 = every N sample calls. ATTENTIONS: The network "
        "might "
