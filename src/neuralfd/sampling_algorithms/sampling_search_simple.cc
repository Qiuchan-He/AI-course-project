#include "neuralfd/sampling_algorithms/sampling_search_simple.h"
...
string SamplingSearchSimple::construct_header() const
{
    ...
    if (store_operator) {
        ...
        for (int idx = 0; idx < task->get_num_operators(); ++idx) {
            oss << task->get_operator_name(idx) << state_separator;
        }
        oss.seekp(-1, oss.cur);
        oss << endl;
    }
...
vector<string> SamplingSearchSimple::extract_samples()
{
    ...
    algorithm->get_search_space().trace_path(
        algorithm->get_goal_state(),
        trajectory);
    Plan plan = algorithm->get_plan();
    ...
    for (size_t idx_t = trajectory.size(); idx_t-- > 0;) {
        ...
        if (store_state) {
            State state =
                algorithm->get_state_registry().lookup_state(trajectory[idx_t]);
...
static shared_ptr<SearchAlgorithm>
_parse_sampling_search_simple(OptionParser& parser)
{
    ...
    shared_ptr<sampling_algorithm::SamplingSearchSimple> algorithm;
    if (!parser.dry_run()) {
        algorithm = make_shared<sampling_algorithm::SamplingSearchSimple>(opts);
    }
...
static Plugin<SearchAlgorithm>
    _plugin_search("sampling_search_simple", _parse_sampling_search_simple);
