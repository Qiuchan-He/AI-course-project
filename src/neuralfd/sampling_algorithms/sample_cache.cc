...
bool operator==(
    const SampleCache::Iterator& lhs,
    const SampleCache::Iterator& rhs)
{
    return lhs.unique == rhs.unique && lhs.iter_vector == rhs.iter_vector &&
           lhs.iter_set == rhs.iter_set;
}
...
{
    if (max_size <= 0) {
        cerr << "The maximum cache size has be positive: " << max_size << endl;
