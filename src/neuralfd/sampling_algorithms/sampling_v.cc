...
prepare_evaluator_parse_tree(const std::string& unparsed_config)
{
...
SamplingV::SamplingV(const options::Options& opts)
    ...
    , evaluator_parse_tree(
          prepare_evaluator_parse_tree(opts.get_unparsed_config()))
...
{
    SamplingStateAlgorithm::initialize();
    cout << "Initializing Sampling Q...";
    cout << "done." << endl;
}

void SamplingV::reload_evaluator(shared_ptr<ClassicalTask> task)
{
    sampling_technique::modified_task = std::move(task);
...
void SamplingV::reload_task(shared_ptr<ClassicalTask> task)
{
    q_task = std::move(task);
    q_task_proxy = make_shared<ClassicalTaskProxy>(*q_task);
    ...
    //    if (expand_goals_cost_limit > 0 && (expanded_goals == nullptr ||
    ...
    //        for (int i = 0; i < task->get_num_goal_facts(); ++i){
    //            FactPair goal_fact = task->get_goal_fact(i);
    //            goal_values[goal_fact.var] = goal_fact.value;
    //        }
    //        PartialAssignment goal_assignment(*task, std::move(goal_values));
    //        auto registry_costs = rrws.sample_area(
    //                goal_assignment, expand_goals_cost_limit,
    //                expand_goal_state_limit, true);
    //        expanded_goals =
    //        make_shared<assignment_cost_generator::AssignmentCostGenerator>(
    //                *q_task_proxy,
    //                registry_costs.first,registry_costs.second);
    //        cout << "Done." << endl;
    //    }
}

std::pair<int, std::vector<StateTree>>
SamplingV::construct_state_tree(const State& state)
{
    ...
    size_t pre_final_layer = std::numeric_limits<size_t>::max();
    size_t i = 0;
    ...
    while (i < states.size() && depth >= 0) {
        if (states[i].value == -1) {
            ...
            const State curr_state =
                q_state_registry->lookup_state(states[i].state_id);
            ...
            if (applicable_ops.empty()) {
                ...
                for (OperatorID op_id : applicable_ops) {
                    ...
                    if (task_properties::is_goal_state(
                        ...
                        //                    } else if (expanded_goals !=
                        //                    nullptr){
                        //                        int c =
                        //                        expanded_goals->lookup_cost(succ_state.unpack());
                        //                        if (c != -1) {
                        //                            states.back().value = c;
                        //                        }
                    }
...
double SamplingV::evaluate_q_value(const State& state)
{
    ...
    pair<int, vector<StateTree>> state_tree = construct_state_tree(state);
    ...
    for (size_t i = state_tree.first; i < state_tree.second.size(); ++i) {
        if (state_tree.second[i].value == -1) {
            eval_contexts.emplace_back(
                q_state_registry->lookup_state(state_tree.second[i].state_id));
        }
    }
    ...
    for (size_t i = state_tree.first; i < state_tree.second.size(); ++i) {
        if (state_tree.second[i].value == -1) {
            state_tree.second[i].value =
                eval_results[idx_eval_results++].get_evaluator_value();
        }
    }
    ...
    for (auto node = state_tree.second.rbegin();
        ...
        if (parent.value == -1) {
            ...
            parent.value =
                std::min(parent.value, node->value + node->action_cost);
        }
    }
    utils::exit_with(utils::ExitCode::SEARCH_CRITICAL_ERROR);
}

string SamplingV::convert_output(const State& state, double q_value)
{
    ostringstream sout;
...
vector<string> SamplingV::sample(shared_ptr<ClassicalTask> task)
{
    ...
    if (q_task == nullptr || (task_reload_frequency != -1 &&
    ...
    const State state =
        q_state_registry->insert_state(task->get_initial_state_values());
...
void SamplingV::add_sampling_v_options(options::OptionParser& parser)
{
    parser.add_option<shared_ptr<Evaluator>>(
        "vevaluator",
        "evaluator to use to estimate the v values");
    ...
    //    parser.add_option<int>(
    //            "expand_goal",
    //            "performs an max x cost breath-first search around the goal.
    //            States" "which within the lookahead which are in the
    //            breath-first search get" "their h* value and are not evaluated
    //            by the network. Use -1 to" "deactivate this (and no genius, do
    //            not use 0 which is equal to" "not eqpanding anything, but
    //            might occure overhead (if I am to " "lazy to check this))",
    //            "0"
    //            );
    //    parser.add_option<int>(
    //            "expand_goal_state_limit",
    //            "limits how many states are searched for the goal
    //            expansions.",
    //            "-1"
    //            );
    //    parser.add_option<bool>(
    //            "reload_expanded_goals",
    //            "reloads the expanded goal when the task is reloaded. By
    //            default" "this is active. Depending on your task
    //            transformations you can" "and should deactivate this, because
    //            the expansion is expensive.", "true"
    //            );
}
} // namespace sampling_algorithm
