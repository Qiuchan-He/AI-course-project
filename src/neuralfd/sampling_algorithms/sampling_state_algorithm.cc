...
static SampleFormat select_sample_format(const string& sample_format)
{
...
static StateFormat select_state_format(const string& state_format)
{
...
SamplingStateAlgorithm::SamplingStateAlgorithm(const options::Options& opts)
...
string SamplingStateAlgorithm::sample_file_header() const
{
    ostringstream oss;
...
void SamplingStateAlgorithm::convert_and_push_state(
    ostringstream& oss,
    const State& state) const
{
    ...
    if (state_format == StateFormat::FDR) {
        const ClassicalTask* task =
            static_cast<ClassicalTaskProxy>(state.get_task()).get_task();
        bool first_round = true;
        for (int var = 0; var < task->get_num_variables(); ++var) {
            for (int val = 0; val < task->get_variable_domain_size(var);
                 ++val) {
...
void SamplingStateAlgorithm::convert_and_push_goal(
    ostringstream& oss,
    const ClassicalTask& task) const
{
    if (state_format == StateFormat::FDR) {
        vector<int> shift;
        shift.push_back(0);
        ...
        for (int var = 0; var < task.get_num_variables(); ++var) {
            ...
            shift.push_back(
                shift[var] + task.get_variable_domain_size(var) - nb_undefined);
        }
...
void SamplingStateAlgorithm::add_sampling_state_options(
