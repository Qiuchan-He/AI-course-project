...
SamplingAlgorithm::SamplingAlgorithm(const options::Options& opts)
    ...
    , sample_cache_manager(SampleCacheManager(
          SampleCache(opts.get<bool>("prune_duplicates")),
          opts.get<int>("sample_cache_size"),
          opts.get<bool>("iterate_sample_files"),
          opts.get<int>("max_sample_files"),
          opts.get<int>("index_sample_files"),
          this))
    , rng(utils::parse_rng_from_options(opts))
{
}

void SamplingAlgorithm::initialize()
{
    cout << "Initializing Sampling Algorithm...";
    remove(plan_manager.get_plan_filename().c_str());
    cout << "done." << endl;
}

void SamplingAlgorithm::update_current_technique()
{
    if (shuffle_sampling_techniques) {
        ...
        int chosen = rng->random(total_remaining);
        for (auto iter = sampling_techniques.begin();
             iter != sampling_techniques.end();
             ++iter) {
...
SearchStatus SamplingAlgorithm::step()
{
    ...
    const shared_ptr<ClassicalTask> next_task =
        (*current_technique)->next(task);
    vector<string> new_samples = sample(next_task);
    sample_cache_manager.insert(new_samples.begin(), new_samples.end());
    return IN_PROGRESS;
}

void SamplingAlgorithm::print_statistics() const
{
    cout << "Generated Entries: " << (sample_cache_manager.size()) << endl;
...
void SamplingAlgorithm::save_plan_if_necessary()
{
    sample_cache_manager.finalize();
}

void SamplingAlgorithm::add_sampling_options(options::OptionParser& parser)
{
    ...
    parser.add_option<bool>(
        ...
        "their probability proportional to the remaining tasks of that "
        "technique.",
        "false");
