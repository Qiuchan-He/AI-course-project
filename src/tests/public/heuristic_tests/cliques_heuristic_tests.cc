...
TEST(CliquesHeuristicTestsPublic, test_bw_goal_aware)
{
    // 4 blocks
    ...
    pdbs::PatternCollection patterns = {
        ...
        {problem.get_variable_location(1)},
        {problem.get_variable_location(2)},
        {problem.get_variable_location(3)}};

    auto task = create_problem_task(problem, initial_state, goal);
    auto hpdb_can = std::make_shared<CliquesHeuristic>(task, patterns);

    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb_can), 0);
}

TEST(CliquesHeuristicTestsPublic, test_bw_single_state)
{
    // 4 blocks
    BlocksWorldProblem problem(4);
    ...
    std::vector<FactPair> initial_state = {
        ...
        problem.get_fact_location_on_table(0),
        problem.get_fact_location_on_block(1, 0),
        problem.get_fact_location_on_block(2, 3),
    ...
    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb_can), 4);
}

TEST(CliquesHeuristicTestsPublic, test_bw_single_state_reeval)
{
    // 4 blocks
    BlocksWorldProblem problem(4);
...
TEST(CliquesHeuristicTestsPublic, test_bw_astar)
{
    // 6 blocks
    ...
    pdbs::PatternCollection patterns = {
        ...
        {problem.get_variable_location(2)},
        {problem.get_variable_location(3)},
        {problem.get_variable_location(4)}};

    auto task = create_problem_task(problem, initial_state, goal);
    auto hpdb_can = std::make_shared<CliquesHeuristic>(task, patterns);
    auto algorithm = create_astar_search_algorithm(task, hpdb_can);

    algorithm->search();

    ASSERT_TRUE(algorithm->found_solution());
    ASSERT_EQ(algorithm->get_statistics().get_reopened(), 0);
    ASSERT_EQ(algorithm->get_statistics().get_expanded(), 586);
...
TEST(CliquesHeuristicTestsPublic, test_gripper_goal_aware)
{
    // 2 rooms, 2 balls
    ...
    pdbs::PatternCollection patterns = {
        {problem.get_variable_robot_at(), problem.get_variable_ball_at(0)},
...
TEST(CliquesHeuristicTestsPublic, test_gripper_single_state)
{
    // 2 rooms, 2 balls
    GripperProblem problem(2, 2);
    ...
    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb_can), 3);
}

TEST(CliquesHeuristicTestsPublic, test_gripper_single_state_reeval)
{
    // 2 rooms, 2 balls
    GripperProblem problem(2, 2);
...
TEST(CliquesHeuristicTestsPublic, test_gripper_astar)
{
    // 2 rooms, 2 balls
    GripperProblem problem(2, 2);
    ...
    ASSERT_TRUE(algorithm->found_solution());
    ASSERT_EQ(algorithm->get_statistics().get_reopened(), 0);
    ASSERT_EQ(algorithm->get_statistics().get_expanded(), 17);
...
TEST(CliquesHeuristicTestsPublic, test_visitall_goal_aware)
{
    // 2x2 grid
    ...
    pdbs::PatternCollection patterns = {
        {problem.get_variable_square_visited(0, 0)},
        {problem.get_variable_square_visited(0, 1)},
...
TEST(CliquesHeuristicTestsPublic, test_visitall_single_state)
{
    // 2x2 grid
    VisitAllProblem problem(2, 2);
...
TEST(CliquesHeuristicTestsPublic, test_visitall_single_state_reeval)
{
    // 2x2 grid
    VisitAllProblem problem(2, 2);
...
TEST(CliquesHeuristicTestsPublic, test_visitall_astar)
{
    // 2x2 grid
    VisitAllProblem problem(2, 2);
    ...
    ASSERT_TRUE(algorithm->found_solution());
    ASSERT_EQ(algorithm->get_statistics().get_reopened(), 0);
    ASSERT_EQ(algorithm->get_statistics().get_expanded(), 6);

    const std::vector<OperatorID> expected_plan = {
        problem.get_operator_move_right_id(0, 0),
...
TEST(CliquesHeuristicTestsPublic, test_sokoban_goal_aware)
{
    ...
    pdbs::PatternCollection patterns = {
        {problem.get_variable_box_at(0), problem.get_variable_box_at_goal(0)},
        {problem.get_variable_box_at(1), problem.get_variable_box_at_goal(1)}};
...
TEST(CliquesHeuristicTestsPublic, test_sokoban_dead_end)
{
    const SokobanGrid playarea = {
        {' ', ' ', ' '},
        {' ', ' ', ' '},
    ...
    ASSERT_EQ(
        get_initial_state_estimate(*task, *hpdb_can),
        Heuristic::DEAD_END);
}

TEST(CliquesHeuristicTestsPublic, test_sokoban_single_state)
{
    const SokobanGrid playarea =
        {{' ', 'G', ' '}, {' ', ' ', ' '}, {' ', ' ', ' '}, {' ', 'G', ' '}};
    ...
    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb_can), 2);
}

TEST(CliquesHeuristicTestsPublic, test_sokoban_single_state_reeval)
{
    const SokobanGrid playarea =
        {{' ', 'G', ' '}, {' ', ' ', ' '}, {' ', ' ', ' '}, {' ', 'G', ' '}};
...
TEST(CliquesHeuristicTestsPublic, test_sokoban_astar)
{
    const SokobanGrid playarea = {
        {' ', ' ', ' '},
        {' ', ' ', 'G'},
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_goal_aware)
{
    // 0 -- 1 -- 2 -- 3
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_dead_end)
{
    ...
    pdbs::PatternCollection patterns = {
        {problem.get_variable_truck_at(), problem.get_variable_package_at(0)},
        {problem.get_variable_package_at(1)}};
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_single_state)
{
    // 4 -- 5 -- 6
    // |  /
    // | /
    ...
    ASSERT_EQ(get_initial_state_estimate(*task, *hpdb_can), 6);
}

TEST(CliquesHeuristicTestsPublic, test_nomystery_single_state_reeval)
{
    // 4 -- 5 -- 6
    // |  /
    // | /
...
TEST(CliquesHeuristicTestsPublic, test_nomystery_astar)
{
    // 4 -- 5 -- 6
    // |  /    /
    // | /    /
