...
GripperProblem::GripperProblem(int num_rooms, int num_balls)
    : ClassicalPlanningProblem(3 + num_balls, (4 * num_balls + num_rooms) * num_rooms)
    , num_rooms(num_rooms)
    , num_balls(num_balls)
{
    // Variables
...
int GripperProblem::get_variable_robot_at() const
{
    return 0;
}

int GripperProblem::get_variable_carry_left() const
{
    return 1;
}

int GripperProblem::get_variable_carry_right() const
{
    return 2;
}

int GripperProblem::get_variable_ball_at(int ball_idx) const
{
    verify_ball_index(ball_idx);
    return 3 + ball_idx;
}

FactPair GripperProblem::get_fact_robot_at_room(int room_idx) const
{
    verify_room_index(room_idx);
    return {get_variable_robot_at(), room_idx};
}

FactPair GripperProblem::get_fact_carry_left_ball(int ball_idx) const
{
    verify_ball_index(ball_idx);
    return {get_variable_carry_left(), ball_idx};
}

FactPair GripperProblem::get_fact_carry_left_none() const
{
    return {get_variable_carry_left(), num_balls};
}

FactPair GripperProblem::get_fact_carry_right_ball(int ball_idx) const
{
    verify_ball_index(ball_idx);
    return {get_variable_carry_right(), ball_idx};
}

FactPair GripperProblem::get_fact_carry_right_none() const
{
    return {get_variable_carry_right(), num_balls};
}

FactPair GripperProblem::get_fact_ball_at_room(int ball_idx, int room_idx) const
{
    verify_room_index(room_idx);
    return {get_variable_ball_at(ball_idx), room_idx};
}

FactPair GripperProblem::get_fact_ball_in_gripper(int ball_idx) const
{
    return {get_variable_ball_at(ball_idx), num_rooms};
}

OperatorID GripperProblem::get_operator_move_id(int r1, int r2) const
{
    return OperatorID(get_operator_move_index(r1, r2));
}

OperatorID GripperProblem::get_operator_pick_left_id(int b, int r) const
{
    return OperatorID(get_operator_pick_left_index(b, r));
}

OperatorID GripperProblem::get_operator_pick_right_id(int b, int r) const
{
    return OperatorID(get_operator_pick_right_index(b, r));
}

OperatorID GripperProblem::get_operator_drop_left_id(int b, int r) const
{
    return OperatorID(get_operator_drop_left_index(b, r));
}

OperatorID GripperProblem::get_operator_drop_right_id(int b, int r) const
{
    return OperatorID(get_operator_drop_right_index(b, r));
}

int GripperProblem::get_operator_move_index(int r1, int r2) const
{
    return r1 + num_rooms * r2;
}

int GripperProblem::get_operator_pick_left_index(int b, int r) const
{
    return num_rooms * num_rooms + b + num_balls * r;
}

int GripperProblem::get_operator_pick_right_index(int b, int r) const
{
    return num_rooms * num_rooms + num_balls * num_rooms + b + num_balls * r;
}

int GripperProblem::get_operator_drop_left_index(int b, int r) const
{
    return num_rooms * num_rooms + 2 * num_balls * num_rooms + b +
           num_balls * r;
}

int GripperProblem::get_operator_drop_right_index(int b, int r) const
{
    return num_rooms * num_rooms + 3 * num_balls * num_rooms + b +
           num_balls * r;
}

void GripperProblem::verify_room_index(int room_idx) const
{
    if (room_idx < 0 || room_idx >= num_rooms) {
        abort_with_error_msg("Room index out of range!");
    }
}

void GripperProblem::verify_ball_index(int ball_idx) const
{
    if (ball_idx < 0 || ball_idx >= num_balls) {
        abort_with_error_msg("Ball index out of range!");
    }
}
