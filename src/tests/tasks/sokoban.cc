...
SokobanProblem::SokobanProblem(
    const std::vector<std::vector<char>>& grid,
    int num_boxes)
    : num_boxes(num_boxes)
{
    if (grid.empty()) {
        abort_with_error_msg("Play area grid is empty!");
    }
...
int SokobanProblem::SquareInfo::get_left_move_index() const
{
    return move_op_indices[0];
}

int SokobanProblem::SquareInfo::get_up_move_index() const
{
    return move_op_indices[1];
}

int SokobanProblem::SquareInfo::get_right_move_index() const
{
    return move_op_indices[2];
}

int SokobanProblem::SquareInfo::get_down_move_index() const
{
    return move_op_indices[3];
}

int SokobanProblem::SquareInfo::get_left_push_index(int crate) const
{
    return push_op_indices[0][crate];
}

int SokobanProblem::SquareInfo::get_up_push_index(int crate) const
{
    return push_op_indices[1][crate];
}

int SokobanProblem::SquareInfo::get_right_push_index(int crate) const
{
    return push_op_indices[2][crate];
}

int SokobanProblem::SquareInfo::get_down_push_index(int crate) const
{
    return push_op_indices[3][crate];
}

int SokobanProblem::to_1D(int x, int y) const
{
    return x + grid_width * y;
}

int SokobanProblem::square_to_index(int x, int y) const
{
    return square_infos[to_1D(x, y)].index;
}

std::pair<int, int> SokobanProblem::index_to_square(int index) const
{
    return playable_squares[index];
}

int SokobanProblem::get_variable_player_at() const
{
    return 0;
}

int SokobanProblem::get_variable_box_at(int box_idx) const
{
    return 1 + box_idx;
}

int SokobanProblem::get_variable_box_at_goal(int box_id) const
{
    return 1 + num_boxes + box_id;
}

int SokobanProblem::get_variable_square_clear(int x, int y) const
{
    verify_square(x, y);
    return 1 + 2 * num_boxes + square_to_index(x, y);
}

FactPair SokobanProblem::get_fact_player_at(int x, int y) const
{
    verify_square(x, y);
    return {get_variable_player_at(), square_to_index(x, y)};
}

FactPair SokobanProblem::get_fact_box_at(int box_idx, int x, int y) const
{
    verify_square(x, y);
...
FactPair
SokobanProblem::get_fact_box_at_goal(int box_idx, bool is_at_goal) const
{
    return {get_variable_box_at_goal(box_idx), is_at_goal ? 1 : 0};
}

FactPair
SokobanProblem::get_fact_square_clear(int x, int y, bool is_clear) const
{
    return {get_variable_square_clear(x, y), is_clear ? 1 : 0};
}

OperatorID SokobanProblem::get_operator_move_left_id(int p_x, int p_y) const
{
    return OperatorID(get_operator_move_left_index(p_x, p_y));
}

OperatorID SokobanProblem::get_operator_move_up_id(int p_x, int p_y) const
{
    return OperatorID(get_operator_move_up_index(p_x, p_y));
}

OperatorID SokobanProblem::get_operator_move_right_id(int p_x, int p_y) const
{
    return OperatorID(get_operator_move_right_index(p_x, p_y));
}

OperatorID SokobanProblem::get_operator_move_down_id(int p_x, int p_y) const
{
    return OperatorID(get_operator_move_down_index(p_x, p_y));
}

OperatorID
SokobanProblem::get_operator_push_left_id(int crate, int p_x, int p_y) const
{
    return OperatorID(get_operator_push_left_index(crate, p_x, p_y));
}

OperatorID
SokobanProblem::get_operator_push_up_id(int crate, int p_x, int p_y) const
{
    return OperatorID(get_operator_push_up_index(crate, p_x, p_y));
}

OperatorID
SokobanProblem::get_operator_push_right_id(int crate, int p_x, int p_y) const
{
    return OperatorID(get_operator_push_right_index(crate, p_x, p_y));
}

OperatorID
SokobanProblem::get_operator_push_down_id(int crate, int p_x, int p_y) const
{
    return OperatorID(get_operator_push_down_index(crate, p_x, p_y));
}

int SokobanProblem::get_operator_move_left_index(int p_x, int p_y) const
{
    return square_infos[to_1D(p_x, p_y)].get_left_move_index();
}

int SokobanProblem::get_operator_move_up_index(int p_x, int p_y) const
{
    return square_infos[to_1D(p_x, p_y)].get_up_move_index();
}

int SokobanProblem::get_operator_move_right_index(int p_x, int p_y) const
{
    return square_infos[to_1D(p_x, p_y)].get_right_move_index();
}

int SokobanProblem::get_operator_move_down_index(int p_x, int p_y) const
{
    return square_infos[to_1D(p_x, p_y)].get_down_move_index();
}

int SokobanProblem::get_operator_push_left_index(int crate, int p_x, int p_y)
    const
{
    return square_infos[to_1D(p_x, p_y)].get_left_push_index(crate);
}

int SokobanProblem::get_operator_push_up_index(int crate, int p_x, int p_y)
    const
{
    return square_infos[to_1D(p_x, p_y)].get_up_push_index(crate);
}

int SokobanProblem::get_operator_push_right_index(int crate, int p_x, int p_y)
    const
{
    return square_infos[to_1D(p_x, p_y)].get_right_push_index(crate);
}

int SokobanProblem::get_operator_push_down_index(int crate, int p_x, int p_y)
    const
{
    return square_infos[to_1D(p_x, p_y)].get_down_push_index(crate);
}

void SokobanProblem::verify_square(int x, int y) const
{
    if (x < 0 || x >= grid_width || y < 0 || y >= grid_height) {
        abort_with_error_msg("Square out of bounds!");
    }

    if (square_to_index(x, y) == -1) {
        abort_with_error_msg("Square in bounds, but not inside the play area!");
    }
}

void SokobanProblem::generate_move_op(
    SquareInfo& sq_info,
    std::string name,
    int dir_index,
    int p_x,
...
void SokobanProblem::generate_push_ops(
    SquareInfo& sq_info,
    std::string name,
    int dir_index,
    int p_x,
