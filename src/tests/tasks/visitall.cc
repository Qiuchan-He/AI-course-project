...
VisitAllProblem::VisitAllProblem(int grid_width, int grid_height)
...
int VisitAllProblem::get_variable_robot_at() const
{
    return 0;
}

int VisitAllProblem::get_variable_square_visited(int x, int y) const
{
    verify_coordinate(x, y);
    return 1 + x + grid_width * y;
}

FactPair VisitAllProblem::get_fact_robot_at_square(int x, int y) const
{
    verify_coordinate(x, y);
    return {get_variable_robot_at(), x + grid_width * y};
}

FactPair
VisitAllProblem::get_fact_square_visited(int x, int y, bool was_visited) const
{
    return {get_variable_square_visited(x, y), was_visited ? 1 : 0};
}

OperatorID VisitAllProblem::get_operator_move_left_id(int x, int y) const
{
    return OperatorID(get_operator_move_left_index(x, y));
}

OperatorID VisitAllProblem::get_operator_move_up_id(int x, int y) const
{
    return OperatorID(get_operator_move_up_index(x, y));
}

OperatorID VisitAllProblem::get_operator_move_right_id(int x, int y) const
{
    return OperatorID(get_operator_move_right_index(x, y));
}

OperatorID VisitAllProblem::get_operator_move_down_id(int x, int y) const
{
    return OperatorID(get_operator_move_down_index(x, y));
}

int VisitAllProblem::get_operator_move_left_index(int x, int y) const
{
    verify_coordinate(x, y);
    verify_coordinate(x - 1, y);
    return (x - 1) + y * (grid_width - 1);
}

int VisitAllProblem::get_operator_move_up_index(int x, int y) const
{
    verify_coordinate(x, y);
    verify_coordinate(x, y + 1);
    return (grid_width - 1) * grid_height + x + y * grid_width;
}

int VisitAllProblem::get_operator_move_right_index(int x, int y) const
{
    verify_coordinate(x, y);
    verify_coordinate(x + 1, y);
    return (grid_width - 1) * grid_height + grid_width * (grid_height - 1) + x +
           y * (grid_width - 1);
}

int VisitAllProblem::get_operator_move_down_index(int x, int y) const
{
    verify_coordinate(x, y);
    verify_coordinate(x, y - 1);
...
void VisitAllProblem::verify_coordinate(int x, int y) const
{
