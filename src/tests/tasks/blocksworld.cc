...
BlocksWorldProblem::BlocksWorldProblem(
    ...
    int pick_cost,
    int put_cost)
    : ClassicalPlanningProblem(2 * num_blocks + 1, 2 * num_blocks * (num_blocks + 1))
...
int BlocksWorldProblem::get_variable_hand_empty() const
{
    return 0;
}

int BlocksWorldProblem::get_variable_location(int block) const
{
    verify_block_index(block);
    return 1 + block;
}

int BlocksWorldProblem::get_variable_clear(int block) const
{
    verify_block_index(block);
    return 1 + num_blocks + block;
}

FactPair BlocksWorldProblem::get_fact_is_clear(int block, bool is_clear) const
{
    return {get_variable_clear(block), is_clear ? 1 : 0};
}

FactPair
BlocksWorldProblem::get_fact_location_on_block(int block, int under_block) const
{
    verify_block_index(under_block);
    return {get_variable_location(block), under_block};
}

FactPair BlocksWorldProblem::get_fact_location_on_table(int block) const
{
    return {get_variable_location(block), num_blocks};
}

FactPair BlocksWorldProblem::get_fact_location_in_hand(int block) const
{
    return {get_variable_location(block), num_blocks + 1};
}

FactPair BlocksWorldProblem::get_fact_is_hand_empty(bool is_empty) const
{
    return {get_variable_hand_empty(), is_empty ? 1 : 0};
}

OperatorID
BlocksWorldProblem::get_operator_id_pick_from_block(int b1, int b2) const
{
    return OperatorID(get_operator_index_pick_from_block(b1, b2));
}

OperatorID BlocksWorldProblem::get_operator_id_pick_from_table(int b) const
{
    return OperatorID(get_operator_index_pick_from_table(b));
}

OperatorID
BlocksWorldProblem::get_operator_id_put_on_block(int b1, int b2) const
{
    return OperatorID(get_operator_index_put_on_block(b1, b2));
}

OperatorID BlocksWorldProblem::get_operator_id_put_on_table(int b) const
{
    return OperatorID(get_operator_index_put_on_table(b));
}

int BlocksWorldProblem::get_operator_index_pick_from_block(int b1, int b2) const
{
    return b1 * num_blocks + b2;
}

int BlocksWorldProblem::get_operator_index_pick_from_table(int b) const
{
    return num_blocks * num_blocks + b;
}

int BlocksWorldProblem::get_operator_index_put_on_block(int b1, int b2) const
{
    return num_blocks * num_blocks + num_blocks + b1 * num_blocks + b2;
}

int BlocksWorldProblem::get_operator_index_put_on_table(int b) const
{
    return num_blocks * num_blocks + num_blocks + num_blocks * num_blocks + b;
}

void BlocksWorldProblem::verify_block_index(int block) const
{
    if (block < 0 || block >= num_blocks) {
        abort_with_error_msg("Usage of non-existent block!");
    }
}
} // namespace tests
