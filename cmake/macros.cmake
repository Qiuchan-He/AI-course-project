...
function(create_fast_downward_library)
    create_library(TARGET downward FLAGS common_cxx_flags ${ARGV})
endfunction()

function(create_probfd_library)
    create_library(TARGET FLAGS common_cxx_flags ${ARGV})
endfunction()

function(create_test_library)
    create_library(TARGET project_tests FLAGS test_cxx_flags ${ARGV})
endfunction()

function(create_library)
    set(_OPTIONS DEPENDENCY_ONLY CORE_LIBRARY)
    set(_ONE_VALUE_ARGS NAME HELP TARGET FLAGS)
    set(_MULTI_VALUE_ARGS SOURCES DEPENDS SOFT_DEPENDS REQUIRES)
    ...
    if (_LIBRARY_SOURCES)
        ...
        endforeach()
        foreach(lib ${_LIBRARY_SOFT_DEPENDS})
            if (TARGET ${lib})
                target_link_libraries(${_LIBRARY_NAME} PUBLIC iface_${lib})
            endif()
        endforeach()
...
function(copy_dlls_to_binary_dir_after_build _TARGET_NAME)
    foreach (dll asmjit;c10;fbgemm;libiomp5md;torch;torch_cpu;uv)
        list(APPEND TORCH_DLLS ${Torch_ROOT}/lib/${dll}.dll)
        list(APPEND TORCHD_DLLS ${Torchd_ROOT}/lib/${dll}.dll)
    endforeach()
    ...
    add_custom_command(
        ...
        "$<IF:$<CONFIG:Debug>,${TORCHD_DLLS},${TORCH_DLLS}>"
