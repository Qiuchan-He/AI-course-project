#ifndef DOWNWARD_OPTIONS_OPTIONS_H
#define DOWNWARD_OPTIONS_OPTIONS_H

#include "downward/options/errors.h"
#include "downward/options/type_namer.h"
...
public:
    ...
    const ParseTree&
    get_parse_tree(const std::string& key, const ParseTree& default_value) const
    {
        const auto it = parse_trees.find(key);
        if (it == parse_trees.end()) {
            return default_value;
