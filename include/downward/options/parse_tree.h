#ifndef DOWNWARD_OPTIONS_PARSE_TREE_H
#define DOWNWARD_OPTIONS_PARSE_TREE_H
...
struct ParseNode {
    ...
    friend std::ostream&
    operator<<(std::ostream& out, const ParseNode& parse_node)
    {
        if (!parse_node.key.empty()) out << parse_node.key << " = ";
...
typename tree<T>::sibling_iterator last_child(
    ...
    typename tree<T>::sibling_iterator tree_it)
{
...
template <class T>
typename tree<T>::sibling_iterator
first_child_of_root(const tree<T>& parse_tree)
{
...
template <class T>
typename tree<T>::sibling_iterator
end_of_roots_children(const tree<T>& parse_tree)
{
...
template <class T>
tree<T>
subtree(const tree<T>& parse_tree, typename tree<T>::sibling_iterator tree_it)
{
