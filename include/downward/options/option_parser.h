#ifndef DOWNWARD_OPTIONS_OPTION_PARSER_H
#define DOWNWARD_OPTIONS_OPTION_PARSER_H
...
class OptionParser {
    ...
    */
    ...
    template <class T>
    void
    check_bounds(const std::string& key, const T& value, const Bounds& bounds);
...
public:
    ...
    void
    document_synopsis(const std::string& name, const std::string& note) const;

    void document_property(const std::string& property, const std::string& note)
        const;
...
inline T TokenParser<T>::parse(OptionParser& parser)
{
    ...
    if ((stream >> std::boolalpha >> x).fail()) {
        parser.error(
            "could not parse argument " + value + " of type " +
...
static std::shared_ptr<T> lookup_in_registry(OptionParser& parser)
{
    const std::string& value = parser.get_root_value();
    try {
        return parser.get_registry().get_factory<std::shared_ptr<T>>(value)(
            parser);
    } catch (const std::out_of_range&) {
        parser.error(
            TypeNamer<std::shared_ptr<T>>::name(parser.get_registry()) + " " +
            value + " not found");
    }
    return nullptr;
}
...
lookup_in_predefinitions(OptionParser& parser, bool& found)
{
...
TokenParser<std::shared_ptr<T>>::parse(OptionParser& parser)
{
    bool predefined;
...
inline std::vector<T> TokenParser<std::vector<T>>::parse(OptionParser& parser)
{
    ...
    for (auto tree_it = first_child_of_root(*parser.get_parse_tree());
        ...
        OptionParser subparser(
            subtree(*parser.get_parse_tree(), tree_it),
            parser.get_registry(),
...
template <class T>
void OptionParser::check_bounds(const std::string&, const T&, const Bounds&)
{
}
    ...
    const std::vector<std::string>& docs)
{
    if (help_mode_) {
        ...
        if (!docs.empty() && docs.size() != names.size()) {
            ABORT(
                "Please provide documentation for all or none of the values "
                "of " +
                key);
        }
    ...
    if (!(stream >> choice).fail()) {
        ...
        // ... otherwise map the string to its position in the enumeration
        // vector.
        auto it =
            find_if(names.begin(), names.end(), [&](const std::string& name) {
                if (name.size() != value.size()) return false;
                for (size_t i = 0; i < value.size(); ++i) {
                    // Ignore case.
                    if (tolower(name[i]) != tolower(value[i])) return false;
                }
                return true;
            });
        if (it == names.end()) {
