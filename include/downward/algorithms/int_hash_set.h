#ifndef DOWNWARD_ALGORITHMS_INT_HASH_SET_H
#define DOWNWARD_ALGORITHMS_INT_HASH_SET_H
...
class IntHashSet {
    ...
    static constexpr int MAX_DISTANCE = 32;
    static constexpr unsigned int MAX_BUCKETS =
...
template <typename Hasher, typename Equal>
struct IntHashSet<Hasher, Equal>::Bucket {
    static constexpr KeyType empty_bucket_key = -1;

    KeyType key;
    HashType hash;

    Bucket()
        : Bucket(empty_bucket_key, 0)
    {
    }
...
template <typename Hasher, typename Equal>
int IntHashSet<Hasher, Equal>::capacity() const
{
    return buckets.size();
}

template <typename Hasher, typename Equal>
void IntHashSet<Hasher, Equal>::rehash(int new_capacity)
{
...
template <typename Hasher, typename Equal>
void IntHashSet<Hasher, Equal>::enlarge()
{
    unsigned int num_buckets = buckets.size();
    // Verify that the number of buckets is a power of 2.
    assert((num_buckets & (num_buckets - 1)) == 0);
    if (num_buckets > MAX_BUCKETS / 2) {
        std::cerr << "IntHashSet<Hasher, Equal> surpassed maximum capacity. "
                     "This means"
                     " you either use IntHashSet<Hasher, Equal> for high-memory"
...
template <typename Hasher, typename Equal>
int IntHashSet<Hasher, Equal>::get_bucket(HashType hash) const
{
    assert(!buckets.empty());
    unsigned int num_buckets = buckets.size();
    // Verify that the number of buckets is a power of 2.
...
*/
template <typename Hasher, typename Equal>
int IntHashSet<Hasher, Equal>::get_distance(int index1, int index2) const
{
...
template <typename Hasher, typename Equal>
int IntHashSet<Hasher, Equal>::find_next_free_bucket_index(int index) const
{
...
template <typename Hasher, typename Equal>
KeyType
IntHashSet<Hasher, Equal>::find_equal_key(KeyType key, HashType hash) const
{
...
*/
template <typename Hasher, typename Equal>
std::pair<KeyType, bool>
IntHashSet<Hasher, Equal>::insert(KeyType key, HashType hash)
{
    assert(hasher(key) == hash);
...
template <typename Hasher, typename Equal>
IntHashSet<Hasher, Equal>::IntHashSet(const Hasher& hasher, const Equal& equal)
    : hasher(hasher)
    , equal(equal)
    , buckets(1)
    , num_entries(0)
...
template <typename Hasher, typename Equal>
IntHashSet<Hasher, Equal>::~IntHashSet() = default;

template <typename Hasher, typename Equal>
int IntHashSet<Hasher, Equal>::size() const
{
    return num_entries;
}
...
*/
template <typename Hasher, typename Equal>
std::pair<KeyType, bool> IntHashSet<Hasher, Equal>::insert(KeyType key)
{
    assert(key >= 0);
...
template <typename Hasher, typename Equal>
void IntHashSet<Hasher, Equal>::dump(utils::LogProxy& log) const
{
    int num_buckets = capacity();
    log << "[";
...
template <typename Hasher, typename Equal>
void IntHashSet<Hasher, Equal>::print_statistics(utils::LogProxy& log) const
{
    assert(!buckets.empty());
    int num_buckets = capacity();
    assert(num_buckets != 0);
    log << "Int hash set load factor: " << num_entries << "/" << num_buckets
        << " = " << static_cast<double>(num_entries) / num_buckets << std::endl;
