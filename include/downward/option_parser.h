#ifndef DOWNWARD_OPTION_PARSER_H
#define DOWNWARD_OPTION_PARSER_H
...
std::shared_ptr<BaseClass> parse(OptionParser& parser)
{
...
template <class BaseClass, class ActualClass>
std::shared_ptr<BaseClass> parse_without_options(OptionParser& parser)
{
    parser.parse();
    if (parser.dry_run()) {
        return nullptr;
