...
public:
    ...
    bool
    equal(const iter& one, const iter& two, const iter& three, BinaryPredicate)
        const;
    template <typename iter>
    ...
    class iterator_base_less {
        using iterator_base =
            typename tree<T, tree_node_allocator>::iterator_base;

    public:
        bool
        operator()(const iterator_base& one, const iterator_base& two) const
        {
...
{
    head = std::allocator_traits<tree_node_allocator>::allocate(
        alloc_,
        1,
        nullptr); // MSVC does not have default second argument
    feet = std::allocator_traits<tree_node_allocator>::allocate(
        alloc_,
        1,
        nullptr);
...
tree<T, tree_node_allocator>::begin_fixed(
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator
tree<T, tree_node_allocator>::end_fixed(
    const iterator_base& pos,
    unsigned int dp) const
{
    assert(
...
iter tree<T, tree_node_allocator>::append_child(iter position, const T& x)
{
    ...
    tree_node* tmp = std::allocator_traits<tree_node_allocator>::allocate(
        alloc_,
        1,
        nullptr);
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::append_children(
    iter position,
    sibling_iterator from,
    sibling_iterator to)
{
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::prepend_children(
    iter position,
    sibling_iterator from,
    sibling_iterator to)
{
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::replace(
    iter position,
    const iterator_base& from)
{
    ...
    if (current_to->next_sibling == nullptr) {
        if (current_to->parent != nullptr) current_to->parent->last_child = tmp;
    } else {
    ...
    sibling_iterator new_end)
{
    ...
    for (;;) {
        pre_order_iterator tt = insert_subtree(
            pre_order_iterator(orig_first),
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::reparent(
    iter position,
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::move_before(
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::sort(
    sibling_iterator from,
    sibling_iterator to,
    bool deep)
{
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::subtree(
    tree& tmp,
    sibling_iterator from,
    sibling_iterator to) const
{
...
template <class T, class tree_node_allocator>
unsigned int
tree<T, tree_node_allocator>::number_of_siblings(const iterator_base& it) const
{
    tree_node* pos = it.node;
    unsigned int ret = 0;
    // count forward
