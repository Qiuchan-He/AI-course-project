#ifndef TASKS_GRIPPER_H
#define TASKS_GRIPPER_H
...
/**
 * @brief A Gripper problem consists of M rooms, N balls and a robot. The robot
 * has two grippers with which he can pick up and drop balls. The robot's task
 * is to transport the balls from one room to another.
 *
 ...
 * @ingroup tasks
 */
class GripperProblem : public ClassicalPlanningProblem {
    int num_rooms;
    int num_balls;

public:
    // Construct a gripper problem instance.
    explicit GripperProblem(int num_rooms, int num_balls);
