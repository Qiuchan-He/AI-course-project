#ifndef TESTS_TASKS_VISITALL_H
#define TESTS_TASKS_VISITALL_H
...
/**
 * @brief A VisitAll problem consists of a 2D grid with fixed width W and fixed
 * height H and a robot inside this grid. The task of the robot is to visit all
 * or some squares of the grid as fast as possible. There is only one action the
 * robot can do: He can move between any two horizontally or vertically, but not
 ...
 * @ingroup tasks
 */
class VisitAllProblem : public ClassicalPlanningProblem {
    int grid_width;
    int grid_height;

public:
    explicit VisitAllProblem(int grid_width, int grid_height);
