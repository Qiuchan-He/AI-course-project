#ifndef NEURALFD_SEARCH_ALGORITHMS_SAMPLING_SEARCH_H
#define NEURALFD_SEARCH_ALGORITHMS_SAMPLING_SEARCH_H
...
protected:
    ...
    const size_t successfully_solved_increment_threshold =
        static_cast<size_t>(successfully_solved_history_size * 0.9);
    ...
    int extract_entries_trajectories(
        ...
        const StateRegistry& sr,
        const OperatorsProxy& ops,
        const std::string& meta_init,
        const std::string& meta_inter,
        const Plan& plan,
    ...
    virtual void next_algorithm() override;
