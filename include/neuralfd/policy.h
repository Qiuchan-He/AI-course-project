#ifndef NEURALFD_POLICY_H
#define NEURALFD_POLICY_H
...
class Policy {
    /*
        ...
        operator_preferences: vector of operator preferences (= probabilities)
       for the same operators with index matching the operator IDs of the
       previous vector
    */
    struct PEntry {
        bool dirty;
        ...
        /*
            non-dirty constructor only using ids (e.g. can be used when all
           operators should have same preference which is handled in
           Policy::compute_result)
        */
        PEntry(std::vector<OperatorID> operator_ids)
            : dirty(false)
            , operator_ids(operator_ids)
            , operator_preferences(std::vector<float>())
        {
        }
        ...
        PEntry()
            : dirty(true)
            , operator_ids(std::vector<OperatorID>())
            , operator_preferences(std::vector<float>())
        {
        }
    };
...
public:
    ...
    virtual PolicyResult compute_result(EvaluationContext& eval_context);
};
